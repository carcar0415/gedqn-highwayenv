...
def extract_structured_obs(road, expected_num, risk_scores, risk_trend=None):
    ...
    for i in range(expected_num):
        ...
        try:
            ego_obs = [
                float(ego.position[0]), float(ego.position[1]), float(ego.speed),
                float(ego.heading), float(ego.lane_index[2] if ego.lane_index else -1),
                float(risk_scores[i]), float(getattr(ego, "target_speed", 15.0)), 
                float(getattr(ego, "acc_value", 0)),
                float(risk_trend[i] if i < len(risk_trend) else 0),
                float(1.0 if getattr(ego, "potential_collision", False) else 0.0),
                float(getattr(ego, "gap", 10.0)),
                float(10.0),  # 左侧边界距离默认值
                float(10.0)   # 右侧边界距离默认值
            ]
            
            # 填充剩余空间
            while len(ego_obs) < 45:
                ego_obs.append(0.0)
                
            structured.append(np.array(ego_obs, dtype=np.float32))
        except Exception as e:
            print(f"处理车辆 {i} 状态时出错: {e}")
            # 提供默认状态
            structured.append(np.zeros(45, dtype=np.float32))
